...
import { _global, getFlag, setFlag, nativeTryCatch } from '@metaweb/utils';
import { SDK_VERSION, SDK_NAME, EVENTTYPES } from '@metaweb/common';
import { InitOptions, VueInstance, ViewModel } from '@metaweb/types';

function init(options: InitOptions) {
  if (!options.dsn || !options.apikey) {
    return console.error(`meta-web 缺少必须配置项：${!options.dsn ? 'dsn' : 'apikey'} `);
  }
