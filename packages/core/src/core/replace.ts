...
import { EVENTTYPES, HTTPTYPE, EMethods } from '@metaweb/common';
import { ReplaceHandler, voidFun } from '@metaweb/types';
...
function xhrReplace(): void {
  ...
  replaceAop(originalXhrProto, 'send', (originalSend: voidFun) => {
    return function (this: any, ...args: any[]): void {
      const { method, url } = this.metaweb_xhr;
      ...
      on(this, 'loadend', function (this: any) {
        ...
        this.metaweb_xhr.requestData = args[0];
        const eTime = getTimestamp();
        // 设置该接口的time，用户用户行为按时间排序
        this.metaweb_xhr.time = this.metaweb_xhr.sTime;
        this.metaweb_xhr.Status = status;
        if (['', 'json', 'text'].indexOf(responseType) !== -1) {
          ...
          if (options.handleHttpStatus && typeof options.handleHttpStatus == 'function') {
            this.metaweb_xhr.response = response && JSON.parse(response);
          }
        }
        // 接口的执行时长
        this.metaweb_xhr.elapsedTime = eTime - this.metaweb_xhr.sTime;
        // 执行之前注册的xhr回调函数
        notify(EVENTTYPES.XHR, this.metaweb_xhr);
      });
